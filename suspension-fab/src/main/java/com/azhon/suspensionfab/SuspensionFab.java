...
//        super.addView(child, index, lp);
    ...
    private void openAnimate() {
        ...
        //在Y轴上的位移值
	    int Y_displacement_1 = 0;
	    int Y_displacement_2 = 0;

        //PGN 展开设计
	    int countOfButton = getChildCount() - 1;
	    if (countOfButton == 4){
	    	for (int i = countOfButton; i > 0; i--){
			    ...
			    if (i >= 3){
				    //fabY轴方向位移的距离
				    Y_displacement_1 += view.getHeight() + fabSpacing;
				    ObjectAnimator animator = ObjectAnimator.ofFloat(view, "translationY", 0f, -Y_displacement_1);
				    viewAnimate(view, animator);
			    }
			    else{
				    Y_displacement_2 += view.getHeight() + fabSpacing;
			    	X_displacement = view.getHeight() + fabSpacing;
				    ObjectAnimator X_animator = ObjectAnimator.ofFloat(view, "translationY", 0f, -Y_displacement_2);
				    viewAnimate(view, X_animator);
				    ObjectAnimator Y_animator = ObjectAnimator.ofFloat(view, "translationX", 0f, -X_displacement);
				    viewAnimate(view, Y_animator);
			    }
		    }
	    }
		else{
    ...
    private void closeAnimate() {
	    ...
	    if (countOfButton == 4){
		    for (int i = 1; i <= countOfButton; i++){
			    ...
			    if (i >= 3){
			    	Y_displacement_1 = view.getHeight() * (countOfButton-i+1) + fabSpacing * (countOfButton-i+1);
				    ObjectAnimator animator = ObjectAnimator.ofFloat(view, "translationY", -Y_displacement_1, 0f);
				    viewAnimate(view, animator);
			    }
			    else{
				    Y_displacement_2 = view.getHeight() * (3-i) + fabSpacing * (3-i);
				    X_displacement = view.getHeight() + fabSpacing;
				    ObjectAnimator X_animator = ObjectAnimator.ofFloat(view, "translationX", -X_displacement, 0f);
				    viewAnimate(view, X_animator);
				    ObjectAnimator Y_animator = ObjectAnimator.ofFloat(view, "translationY", -Y_displacement_2, 0f);
				    viewAnimate(view, Y_animator);
			    }
		    }
	    }
    ...
    private void setLayoutHeightOrWidth() {
        ...
        if (currentState) {
        	if(getChildCount() - 1 == 4){
		        int height = view.getHeight() * 3 + fabSpacing * 2;
		        int width = view.getHeight() * 2 + fabSpacing;
